...
const BookWorkSpace = ({ }) => {
  ...
  }, [viewBy, startTime, endTime, selectedLocation, bookDate]);
  ...
  useEffect(() => {
    if (viewBy != ViewByIds.team) {
      ...
      setWorkSpaceDetails({ ...workSpaceDetails, assetInfo: filteredList });
    } else {
      setWorkSpaceDetails(workSpaceDetailCopy);
    }
  }, [selectedFilter]);
        ...
        (successRes, res) => {
          ...
          if (res?.data?.code == '422') {
            ...
            if (successRes == 'error') {
              ...
              viewBy == ViewByIds.month
                ? setWorkSpaceDetails({ ...successRes, assetInfo: sortedData })
                : setWorkSpaceDetails({
                    ...successRes,
                    team_details: sortedData,
                  });
              // setWorkSpaceDetails(successRes)
  ...
  const createOrUpdateWorkspace = opt => {
  ...
  const onSearchChange = val => {
    ...
    if (val?.trim() !== '' && workSpaceCopy?.team_details?.length > 0) {
      ...
      if (lists?.length > 0) {
        ...
        setWorkSpaceDetails(workSpaceCopy);
  ...
  const getTeamCount = (teamId, date, teamMembersList) => {
    ...
    let selectBookings = userIds?.map(id => {
      let checkData = workSpaceDetails?.booking_details?.find(
  ...
  const getAssetCount = date => {
    let count = workSpaceDetails?.assetInfo?.filter(
      i => i?.asset_status != 3,
    )?.length;
    let assetIds = workSpaceDetails?.assetInfo?.map(i => i?.asset_id);
    let selectBookings = assetIds?.map(id => {
      let checkData = workSpaceDetails?.bookingInfo?.[id]?.find(
      ...
      if (validateOnBehalfOfUser() && data?.status == 'Unknown') {
        const preparData = {
          data: {
            ...data,
            team_name: teamName,
            selectedAssets: assetsList[0].id,
            start_time: startTime,
            end_time: endTime,
          },
        ...
        <div className="new-booking-table">
          <div className="booking-table">
            ...
            {viewBy == ViewByIds.team ? (
              <>
                {workSpaceDetails?.team_details &&
                  workSpaceDetails?.team_details?.length > 0 &&
                  workSpaceDetails?.team_details?.map((team, index) => {
                    return (
                      <div key={index}>
                                          ...
                                          return (
                                            ...
                                            >
                                              <BookingDeskList
                                                ...
                                                monthDetails={opt}
                                                teamBookingDetails={
                                                  workSpaceDetails?.booking_details &&
                                                  workSpaceDetails
                                                    ?.booking_details?.length >
                                                    0 &&
                                                  workSpaceDetails?.booking_details?.find(
                                                    book =>
                                                      book?.user_id ==
                                                        details?.id &&
                                                      book?.booking_date ==
              ...
              <>
                {workSpaceDetails?.assetInfo &&
                  workSpaceDetails?.assetInfo?.length > 0 && (
                    <div className="booking-details">
                      <div className="booking-inner-content">
                        <p>
                          <span>{'All'}</span>
                ...
                {workSpaceDetails?.assetInfo &&
                  workSpaceDetails?.assetInfo?.length > 0 &&
                  workSpaceDetails?.assetInfo?.map((asset, index) => {
                    return (
                      <div key={index} className="booking-details">
                        <div className="booking-inner-content">
                          <p>
