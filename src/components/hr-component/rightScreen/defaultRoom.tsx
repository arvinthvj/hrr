...
const DefaultRoom = ({ checkIsOpned }) => {
  ...
  const [defaultBuliding, setDefaultBuliding] = useState<any | undefined>(
    preferenceDetails?.[0]?.default_room_building,
  );
  ...
  const [floorId, setFloorId] = useState<any>(
    preferenceDetails?.[0]?.default_room_location_id || '',
  );
  ...
  const getFieldValue = () => {
    const workSpaceValue = {
      label: preferenceDetails?.[0]?.default_room_name,
      value: preferenceDetails?.[0]?.default_room_name,
    };
  ...
  const getAllRoomDetails = () => {
    ...
    const payload = {
      location_id: floorId,
      floor_plan_type_id: FloorType.ROOM,
    };
  ...
  useEffect(() => {
    setFloorId(editData?.default_room_location_id);
  }, [editData]);
  ...
  const updateData = () => {
    if (editData && Object.keys(editData)?.length > 0) {
      ...
      assestList?.find(
        ele =>
          ele.value == editData?.default_room_name &&
