...
const HeatmapchartComponent = props => {
  ...
  useEffect(() => {
    if (props?.data?.data?.length > 0) {
      ...
      arr.map((idx, ele) => {
        rows[idx].unshift(daysArray[idx]);
      });

      let keyArray = [];
      keyArray = ['daysofweek', ...categoriesArray];

      const formattedData = Object.keys(rows).map(key => {
        const obj = { daysofweek: rows[key][0] };
      ...
      const formattedDatafinal = formattedData.map(item => {
        const formattedItem = { daysofweek: item.daysofweek };
  ...
  function googlesheetmethodcall() {
    if (props?.data?.data) {
      ...
      let row_header = [];
      row_header = ['Days of Week', ...categoriesArray];
      ...
      const keyArray = ['Days of Week', ...categoriesArray];

      const formattedData = Object.keys(rows).map(key => {
        const obj = { ['Days of Week']: rows[key][0] };
        for (let i = 1; i < keyArray.length; i++) {
          obj[keyArray[i]] = rows[key][i];
        }
        return obj;
      ...
      const formattedDatafinal = formattedData.map(item => {
        const formattedItem = { ['Days of Week']: item['Days of Week'] };
        for (const key in item) {
          if (key !== 'Days of Week') {
            formattedItem[key] = item[key];
          }
